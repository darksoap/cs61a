...
def size(w):
    ...
    "*** YOUR CODE HERE ***"
    assert is_weight(w), "must call a weight on a side"
    return label(w)
...
def balanced(m):
    ...
    if is_weight(m):
        return size(m)
    left_torque, right_torque = [length(x)*total_weight(end(x)) for x in sides(m)]
...
def make_withdraw(balance, password):
    ...
    """
    "*** YOUR CODE HERE ***"
    incorrect_times,incorrect_password = 0, []

    def withdraw(amount, input_password):
        nonlocal balance, incorrect_password, incorrect_times
